...
const HomePage = () => {
  ...
  const hasFetchPokemonError = useMemo(() => {
    if (fetchLoadablePokemon.state === 'hasError' || getLodablePokemons.state === 'hasError') {
      return true;
    } else {
      return false;
    }
  ...
  useEffect(() => {
    if (getLodablePokemons.state === 'hasValue' && getLodablePokemons.contents !== undefined) {
  ...
  useEffect(() => {
    if (getLoadablePokemon?.state === 'hasValue' && getLoadablePokemon?.contents !== undefined) {
  ...
  return (
    <Container>
      <FlexBox align="flex-start" justify="flex-start" direction="column" gap="xxxs">
        <InitialPokemons />
        <PokemonSearch />
        <SinglePokemon
          pokemon={pokemon}
      ...
      <PokemonsSection
        ...
        loading={getLodablePokemons.state === 'loading' || fetchLoadablePokemon.state === 'loading'}
